...
{
    //開始傳送檔案
    fprintf(stderr, GREEN"Start send file!\n"NONE);
...
void pthread_recv(void* ptr)
{
    while(1)
    {
        ...
        if(recv_flag == 2){
	        ...
	        fclose(fp1);
            recv_flag = 0;
            printf(GREEN"download file finished.\n"NONE);
            continue;
        }
        printf("%s",recvbuf);
    ...
    while(1)
    {
        ...
        while (recv_flag == 1)
        {
            ...
            if (strcmp(p, "Y") == 0){
                ...
                printf(GREEN"Please type Y/N : "NONE);
